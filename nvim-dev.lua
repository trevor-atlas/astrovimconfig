RELOAD "user.nvim-dev"

local Job = require "plenary.job"
local plenary_window = require "plenary.window.float"

local nnoremap = function(lhs, rhs) vim.keymap.set('n', lhs, rhs, {}) end
...
nvim_dev.run_this_test = function()
  ...
  local command = string.format("TEST_FILTER=%s TEST_FILE='%s' make functionaltest", vim.fn.shellescape(test_filter),
                                test_file)
...
nvim_dev.run_this_file = function()
  local test_file = vim.fn.expand "%:p"
  local args = "BUSTED_ARGS='--no-keep-going' TEST_FILE=%s make functionaltest"
  local command = string.format(args, vim.fn.shellescape(test_file))
